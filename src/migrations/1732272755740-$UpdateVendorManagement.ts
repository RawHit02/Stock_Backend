...
export class  $UpdateVendorManagement1732272755740 implements MigrationInterface {
    name = ' $UpdateVendorManagement1732272755740'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "StockManagementTbl" ("Id" uuid NOT NULL DEFAULT uuid_generate_v4(), "CreatedBy" character varying(150) NOT NULL, "CreatedDate" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "UpdatedBy" character varying(150), "UpdatedDate" TIMESTAMP WITH TIME ZONE DEFAULT now(), "IsDeleted" boolean NOT NULL DEFAULT false, "StockType" "public"."StockManagementTbl_stocktype_enum" NOT NULL DEFAULT 'inward', "TransId" character varying NOT NULL, "Description" text NOT NULL, "ItemType" character varying(100) NOT NULL, "Quantity" integer NOT NULL DEFAULT '0', "UnitPrice" numeric(20,2) NOT NULL DEFAULT '0', "Commission" numeric(20,2) NOT NULL DEFAULT '0', "TotalValue" numeric(20,2) NOT NULL DEFAULT '0', "BatchNumber" character varying(50) NOT NULL, "Location" character varying(100) NOT NULL, "Notes" text DEFAULT 'N/A', "VendorId" uuid NOT NULL, CONSTRAINT "UQ_602abff21b7105c5ae03f072e2f" UNIQUE ("TransId"), CONSTRAINT "PK_75d1c30e1f5f324d4717e031d33" PRIMARY KEY ("Id"))`);
        await queryRunner.query(`CREATE INDEX "IDX_87de9ee8c98d9e551e32d13ee6" ON "StockManagementTbl" ("CreatedDate") `);
        await queryRunner.query(`ALTER TABLE "StockManagementTbl" ADD CONSTRAINT "FK_114097e40e7016197dd2f07e230" FOREIGN KEY ("VendorId") REFERENCES "VendorManagementTbl"("Id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
